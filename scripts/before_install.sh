...
if [[ ! -f /usr/local/src/openssl-1.1.1l.tar.gz ]] || ! ls -ltra /usr/bin/python3 | grep -E "\-> .*python3\.9"
then
    ...
    rm -r /usr/local/src/openssl-1.1.1l
    rm /usr/local/src/openssl-1.1.1l.tar.gz
    rm -r /usr/local/src/Python3.9.7
    rm /usr/local/src/Python3.9.7.tar.gz

    cd /usr/local/src
    sudo wget https://www.openssl.org/source/openssl-1.1.1l.tar.gz
    sudo tar -xf openssl-1.1.1l.tar.gz
    cd openssl-1.1.1l
    sudo ./config --prefix=/usr/local/ssl --openssldir=/usr/local/ssl shared zlib
    sudo make
    sudo make install
    sudo bash -c 'echo "/usr/local/ssl/lib" > "/etc/ld.so.conf.d/openssl-1.1.1l.conf"'
    sudo ldconfig -v
